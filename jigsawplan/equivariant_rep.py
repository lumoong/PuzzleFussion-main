...
if __name__=="__main__": 
    ...
    # rho-theta 좌표계의 2D 배열 예제

    # rho-theta 좌표계를 카르테시안 좌표계로 변환
    ...
    def update(frame):
        ...
        ax.set_aspect('equal', 'box')
        rho = 1.0
        # theta 값 (x축과의 각도, 라디안 단위)
        zzz=4
            
        thetas = np.linspace(0, 2*np.pi, zzz, endpoint=False)-np.pi
        thetas += np.pi *frame/20
        '''thetas[0]+=np.pi/8
        thetas[2]+='''
        #thetas[2]+=np.pi/16
        import random
        '''k=random.randint(0,15)
        thetas[:k],thetas[k:]=thetas[zzz-k:], thetas[:zzz-k]'''


        
        while (1):
            if np.all(thetas<=np.pi):
                break
            thetas[thetas>np.pi]-=2*np.pi
        # rho-theta 좌표계로 정 8각형의 꼭지점 좌표 생성
        rho_theta_coordinates = np.column_stack((np.full_like(thetas, rho), thetas))
        #rho_theta_coordinates[::2, 0]=0.5
        #rho_theta_coordinates[13, 0]=6.0

        
        A=ordered_cyclic(rho_theta_coordinates)
        R, U , targetarrow = A.group()

        x = rho_theta_coordinates[:, 0] * np.cos(rho_theta_coordinates[:, 1])
        y = rho_theta_coordinates[:, 0] * np.sin(rho_theta_coordinates[:, 1])
        cartesian_coordinates = np.column_stack((x, y))

        polygon = patches.Polygon(cartesian_coordinates, closed=True, fill=None, edgecolor='r')
        ax.add_patch(polygon)

        specific_rho_theta=A.nump[targetarrow]
        specific_x = specific_rho_theta[0] * np.cos(specific_rho_theta[1])
        specific_y = specific_rho_theta[0] * np.sin(specific_rho_theta[1])
        arrow = patches.FancyArrow(0, 0, specific_x, specific_y, head_width=0.1, head_length=0.1, fc='blue', ec='blue')
    ...
    # 애니메이션 생성
    ani = FuncAnimation(fig, update, frames=range(0, 300), blit=True)
    plt.show()
